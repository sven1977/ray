...
from ray.rllib.components.distributions import Categorical, MultiCategorical
...
class TestDistributions(unittest.TestCase):
    ...
    def test_categorical(self):
        # Create a categorical distribution of 3 categories.
        input_space = Box(-1.0, 2.0, (10, 3), np.float32)

        input_ = input_space.sample()

        categorical_tf = Categorical(input_)
        categorical_torch = Categorical(input_, framework="torch")
        ...
        for _ in range(10):
            out = categorical_tf.deterministic_sample()
            check(out, expected)
            out = categorical_torch.deterministic_sample()
            check(out, expected)

        # Batch of size=3 and non-deterministic -> expect roughly the mean.
        input_ = input_space.sample()
        categorical = Categorical(input_)
        outs = []
        for _ in range(100):
        ...
        expected = np.log(
            np.array([probs[i][values[i]] for i in range(10)])
        )
