...
class MultiAgentEnvRunner(EnvRunner):
    """An environment runner to collect data from vectorized MultiAgentEnv objects."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            config: The config to use to set up this MultiAgentEnvRunner.
        """
        ...
        # Create the connector pipelines: Env-to-module and module-to-env.
        self.env_to_module_connector = None
        self.module_to_env_connector = None

        # Create the MARLModule to use for action computations.
        ...
        self.marl_module = module_spec.build()
        ...
        *,
        ...
        Args:
            ...
            force_reset: Whether to reset the environment(s) before starting to sample.
                Only relevant if `num_timesteps` is not None (and `num_episodes` is None).
                Note, even is this is False, we will still reset the environment(s) if
                they were left in a terminated or truncated state during previous calls
                to `sample()`.
            with_render_data: If True, will record rendering images per timestep
                ...
                reports.
                TODO (sven): Note that this is only supported for running with
                 `num_episodes` yet.
    ...
    def _sample_timesteps(
        ...
        initial_states = tree.map_structure(
            ...
            self.marl_module.get_initial_state(),
        )
        ...
        if force_reset or self._needs_initial_reset:
            ...
            self._episodes = [MultiAgentEpisode() for _ in range(self.num_envs)]
        ...
        while ts < num_timesteps:
            ...
            # Compute an action using our RLModule.
                ...
                # Explore or not.
                if explore:
                    outs = self.marl_module.forward_exploration(batch)
                else:
                    outs = self. marl_module.forward_inference(batch)
            ...
            for i in range(self.num_envs):
                ...
                if terminateds[i] or truncateds[i]:
                    ...
                    # new episode.
                    for k, v in self.marl_module.get_initial_state().items():
                        states[k][i] = v.numpy()
                    is_first[i] = True
                    done_episodes_to_return.append(self._episodes[i])
                    # Create a new episode object.
                    self._episodes[i] = MultiAgentEpisode(observations=[obs[i]], states=s)
                else:
                    self._episodes[i].add_timestep(
    ...
    def _sample_episodes(
        ...
        obs, _ = self.env.reset()
        episodes = [MultiAgentEpisode() for _ in range(self.num_envs)]
    ...
    def set_weights(self, weights, global_vars=None):
        ...
        self.marl_module.set_state(weights[DEFAULT_POLICY_ID])
